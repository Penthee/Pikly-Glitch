...
namespace TeamUtility.IO
{
	...
	public sealed class InputAdapter : MonoBehaviour
	{
		...
		public static Vector3 mousePosition
		{
			get
			{
				return InputMgr.mousePosition;
			}
		}
		
		public static float GetAxis(string axisName)
		{
			return InputMgr.GetAxis(axisName);
		}
		...
		public static float GetTriggerAxis(InputTriggerAxis axis)
		{
			...
			{
				return InputMgr.GetAxis(_instance.leftTriggerAxis);
			}
			else
			{
				return InputMgr.GetAxis(_instance.rightTriggerAxis);
			}
		}
		...
		public static float GetDPADAxis(InputDPADAxis axis)
		{
			...
			{
				return InputMgr.GetAxis(_instance.dpadHorizontalAxis);
			}
			else
			{
				return InputMgr.GetAxis(_instance.dpadVerticalAxis);
			}
		}
		
		public static bool GetButton(string buttonName)
		{
			return InputMgr.GetButton(buttonName);
		}
		...
		{
			return InputMgr.GetButtonUp(buttonName);
		}
		
		public static bool GetMouseButton(int button)
		{
			return InputMgr.GetMouseButton(button);
		}
		
		public static bool GetMouseButtonDown(int button)
		{
			return InputMgr.GetMouseButtonDown(button);
		}
		
		public static bool GetMouseButtonUp(int button)
		{
			return InputMgr.GetMouseButtonUp(button);
		}
		...
		{
			InputMgr.ResetInputConfiguration(PlayerID.One);
            InputMgr.ResetInputAxes();
		}
		
		public static string[] GetJoystickNames()
		{
			return InputMgr.GetJoystickNames();
		}
		...
		private void Awake()
		{
			...
			else
			{
				...
				_instance = this;
				_joystickCount = InputMgr.GetJoystickNames().Length;
		...
		private void Update()
		{
            ...
            InputMgr.lastMousePosition = InputMgr.mousePosition;
		}
		...
		private void UpdateTriggerAndDPAD()
		{
			if(_inputDevice == InputDevice.Joystick)
			{
				...
				_currentDpadValues.x = InputMgr.GetAxis(_instance.dpadHorizontalAxis);
				_currentDpadValues.y = InputMgr.GetAxis(_instance.dpadVerticalAxis);
				_lastTriggerValues = _currentTriggerValues;
				_currentTriggerValues.x = InputMgr.GetAxis(_instance.leftTriggerAxis);
				_currentTriggerValues.y = InputMgr.GetAxis(_instance.rightTriggerAxis);
			}
			else
			{
		...
		private void UpdateInputDevice()
		{
			...
			if(_inputDevice == InputDevice.Joystick)
			{
                if (InputMgr.mousePosition != InputMgr.lastMousePosition) {
                    SetInputDevice(InputDevice.KeyboardAndMouse);
                }
				...
				if(InputMgr.anyKey)
				{
					for(int i = 0; i <= 19; i++)
					{
						if(InputMgr.GetKey((KeyCode)(_firstJoystickKey + i)))
						{
							anyJoystickKey = true;
							break;
						}
				...
				} else if(InputMgr.AnyInput(_keyboardConfiguration)) {
					SetInputDevice(InputDevice.KeyboardAndMouse);
                }
			}
			else
			{
				...
				{
					if(InputMgr.AnyInput(_joystickConfiguration))
					{
						SetInputDevice(InputDevice.Joystick);
					}
				}
		...
		private void SetInputDevice(InputDevice inpuDevice)
		{
			...
			if(inpuDevice == InputDevice.Joystick)
			{
				...
				Cursor.visible = false;
#endif
                InputMgr.SetInputConfiguration(_joystickConfiguration, PlayerID.One);
			...
			else
			{
				...
				Cursor.visible = true;
#endif
                InputMgr.SetInputConfiguration(_keyboardConfiguration, PlayerID.One);
