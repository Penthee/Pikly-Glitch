...
namespace Pikl.Utils.RDS {
    ...
    public static class RDSRandom {
        ...
        /// <summary>
        /// Retrieves the next float random value from the random number generator.
        /// The result is always between the given min-value (including) and the given max-value (excluding).
        /// </summary>
        /// <param name="min">The minimum value.</param>
        /// <param name="max">The maximum value.</param>
        ...
        public static int Int(int min, int max) {
            if (min > max)
                throw new ArgumentOutOfRangeException("min and max", "the max value (" + max + ") is smaller than the min value (" + min + ").");
