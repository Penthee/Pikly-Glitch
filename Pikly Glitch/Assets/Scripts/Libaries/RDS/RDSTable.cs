...
namespace Pikl.Utils.RDS {
    ...
    public class RDSTable : IRDSTable {
        ...
        internal List<IRDSObject> mcontents = null;
        ...
        public virtual IRDSObject AddEntry(IRDSObject entry) {
            mcontents.Add(entry);
            entry.rdsTable = this;
            return entry;
        }
        ...
        #region RESULT
        //private List<IRDSObject> uniquePerResultDrops = new List<IRDSObject>();
        private List<IRDSObject> uniqueForeverDrops = new List<IRDSObject>();

        private void AddToResult(List<IRDSObject> rv, IRDSObject o) {
            //(o.rdsAmount > 0 || o.rdsAmount == -1) && 
            if (!(o.rdsUnique && uniqueForeverDrops.Contains(o))) {
                //if (!uniquePerResultDrops.Contains(o))
                //    uniquePerResultDrops.Add(o);
                if (o.rdsAmount > 0) {
                    o.rdsAmount--;
                    //UnityEngine.Debug.Log(o.rdsName + "'s amount is " + o.rdsAmount);
                }
                if (o.rdsAmount == 0)
                    o.rdsEnabled = false;

                if (!uniqueForeverDrops.Contains(o) && o.rdsUnique)
                    uniqueForeverDrops.Add(o);
                ...
                UnityEngine.Debug.Log("skipping " + o.ToString());
            }
        }
        ...
        public virtual IEnumerable<IRDSObject> rdsResult {
            get {
                ...
                //uniquePerResultDrops = new List<IRDSObject>();
                ...
                if (realdropcnt > 0) {
                    for (int dropcount = 0; dropcount < realdropcnt; dropcount++) {
                        ...
                        IEnumerable<IRDSObject> dropables = mcontents.Where(e => e.rdsEnabled && !e.rdsAlways && (e.rdsAmount > 0 || e.rdsAmount == -1));

                        // This is the magic random number that will decide, which object is hit now
                        double hitvalue = RDSRandom.Double(dropables.Sum(e => e.rdsProbability));
        ...
        /// </summary>
        public int rdsAmount { get; set; }
        /// <summary>
        /// Gets or sets whether this object may be contained in a result set once
        /// </summary>
